# Here — configurador de OwnTracks para CloserClick

**`here.closer.click`** es el **configurador** del pilar **geo** del ecosistema
[CloserClick](https://closer.click/): arma **círculos** privados de ubicación,
...
1. **Círculos** — listar / crear círculos; agregar miembros desde los
   **contactos del vault** (`id.closer.click`). Cada círculo tiene una **clave
...
`closer-click-geo`, ruta `POST /here`, cerrado y aditivo). `here` no llama al
...
| Campo            | Valor                                             |
|------------------|---------------------------------------------------|
| URL              | `https://geo.closer.click/here`                   |
| Auth             | HTTP Basic                                         |
| username         | `circleId`                                         |
...
- **`@closerclick/closer-click-identity` v0.10.0** — única fuente de identidad:
  - `Identity.connect()` (vault iframe), `signData`, `signDelegation`,
    `revokeDelegation`, `listDelegations`, `listContacts`.
  - Subpath ESM puro `@closerclick/closer-click-identity/capabilities`:
    `makeDeviceKey`, `pubkeyId`.
- **`@closerclick/closer-click-support`** — moneda de soporte (§6).
- **`@closerclick/closer-click-nav`** — botón "volver" unificado.
...
- **Cifrado y reparto de la clave del círculo** — `src/lib/circles.js`
  ...
  key)` y repartirla por el **proxy** (`@closerclick/closer-click-proxy-client`) o
...
- **Llamada al bridge** — `here` no la hace (OwnTracks publica). El bridge es la
  ruta `POST /here` en `closer-click-geo` (cerrado, aditivo). Marcado
  `TODO(bridge)`.
- **Persistencia en el store** — `@closerclick/closer-click-store` en lugar del
  repo en memoria de `src/lib/circles.js`. Marcado `TODO(store)`.
- **Perfil** — montar `<closer-click-profile mode="self">` con
...
Repo en la org **`closerclick/`**, subdominio **`here.closer.click`** (GitHub
...
> **NO se ha registrado todavía en el catálogo** (`closerclick/src/App.vue`),
> a propósito (este scaffold no toca ese repo). Para lanzar la app hay que, según
> `CONVENCIONES-APPS.md` §11.4:
>
> 1. Copiar `public/icon.svg` → `closerclick/src/assets/apps/here.svg`.
> 2. En `closerclick/src/App.vue`: `import hereLogo from './assets/apps/here.svg'`
...
Parte del ecosistema **CloserClick**. Ko-fi:
[ko-fi.com/closerclick](https://ko-fi.com/closerclick) ·
